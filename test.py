...
#    array = np.delete(array, 2, 1)
    # print(array)
    df = pd.DataFrame(array, columns=['time', 'AQI', 'lat', 'lon'])
...
ADO_PATH = r'Data sample/AOD/'
PM25_PATH = r'Data sample/'
RH_PATH = r'Data sample/RH/'
...
data = readAERONET(AERONET_PATH + 'HoChiMinhCity_PM2.5_2017_YTD.csv')
data.to_csv(AERONET_PATH + 'PM2.5 HCM 2017.csv', index=False)
############################################################################################


############################################################################################
# read the interpolated data file
AOD_550 = pd.read_csv(AERONET_PATH+ 'PM2.5 HCM 2017.csv',
                      parse_dates=['time'],
...
for idx, row in mean_by_day.iterrows():
    ...
    try:
        img = gdal.Open( 'AOD/' + str(int(row['year'])) + '/MAIAC_composite_' + name + '.tif')
...
print(mean_by_day)
print(img_data)
mean_by_day.to_csv(AERONET_PATH + 'PM2.5 vs MAIAC daily HCM 2017.csv', index=False)
############################################################################################
